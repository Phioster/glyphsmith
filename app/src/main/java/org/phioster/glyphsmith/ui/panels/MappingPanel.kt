...
/**
 * Everything between a cell's luminance and the level it ends up with: the tone curve, the
 * pre-dither adjustments, and the dithering that spreads quantisation error across neighbours.
 *
 * The panel is in two halves, and only the second one is about glyphs. [DitherMappingSection]
 * is what the levels are decided by and applies to every render mode; [EdgeMappingSection]
 * swaps a brightness-matched glyph for a direction-matched one and applies only where a module
 * produces glyphs. [MappingSections] holds that rule, so this file only asks.
 */
@Composable
fun MappingPanel(
    ...
    Column(modifier.fillMaxWidth()) {
        ...
        TerminalButton(
            label = "reset mapping",
            onClick = { onChange(MappingSections.reset(params)) },
...
/**
 * The render-neutral half: tone, the adjustments that run before the dither, the dither style
 * and whatever that style itself asks for.
 *
 * Nothing here knows what a level is finally drawn as, which is why it is offered in pixel
 * dither, glyph art and the chained mode alike.
 */
@Composable
private fun DitherMappingSection(
    params: RenderSettings,
    onChange: (RenderSettings) -> Unit,
    favourites: Set<String>,
    onToggleFavourite: (String) -> Unit,
    ...
    Text(
        "order: gamma → contrast → brightness → midtones → highlights, then the dither",
        color = Term.InkFaint,
    ...
    Text(
        "these run before the dither, so they change what the algorithm sees and the " +
            ...
            "only change how the finished render looks. denoise drops stray cells without " +
        ...
        if (!Dither.usesPattern(params.ditherMode)) {
            ...
            Text(
                "spreads the rounding error onto neighbouring cells — this is what makes " +
                    "a handful of levels read as a gradient instead of bands",
                color = Term.InkFaint,
                style = MaterialTheme.typography.bodySmall,
    ...
    if (Dither.isModulation(params.ditherMode)) {
        ModulationControls(params, onChange)
    }
}

/**
 * The settings the modulation styles add: one period, one second axis whose name belongs to the
 * style, an angle, a phase — and, for the two orb styles, the shape of an individual orb.
 */
@Composable
private fun ModulationControls(params: RenderSettings, onChange: (RenderSettings) -> Unit) {
    TerminalSlider(
    ...
    if (params.ditherMode == DitherMode.MOD_ORB || params.ditherMode == DitherMode.BEEHIVE) {
        SectionHeader("orb")
...
/**
 * The glyph half: edge detection, and which glyphs a detected edge is drawn with.
 *
 * Shown only where a module produces glyphs. The edge grid is computed in the shared quantise
 * pass but read by nothing except the glyph engine, so in pixel dither these controls would
 * change the render by exactly nothing.
 */
@Composable
private fun EdgeMappingSection(params: RenderSettings, onChange: (RenderSettings) -> Unit) {
    SectionHeader("edge glyphs")
